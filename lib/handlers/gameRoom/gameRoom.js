...
const gameRoom = (io, socket) => {
  ...
  socket.on('START_GAME', roomKey => {
    ...
    // diffNumber scales allowedTime
    let diffNumber = 1;

    if(room.difficulty === 'medium') diffNumber = 2;
    if(room.difficulty === 'hard') diffNumber = 3;

    // Convert easy/medium/hard to something Math Problems API can understand
    // Then get problems and start the game
    getProblemSet(difficulty[room.difficulty])
      .then(problems => io.to(roomKey).emit('PROBLEM_SET', problems))
      .then(
        toNextCard(() => {
          roundLogic({ allowedTime: 30 * diffNumber, roundString: 'ROUND_ONE' });
        }, 5)
      );
  });
  ...
  socket.on('GAME_OPTIONS', ({ difficulty, roomKey }) => {
    ...
    io.to(roomKey).emit('GAME_OPTIONS_RESULTS', difficulty);
  });
};
